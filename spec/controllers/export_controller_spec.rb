  ...
  describe "GET /export/ddahs/:position_id" do
    context "when expected" do
      it "returns a CSV file" do
        offer.reload
        get :ddahs, params: {position_id: position[:id]}
        expect(response.status).to eq(200)
        expect(response.content_type).to eq("text/csv")
        expect(response.header["Content-Disposition"]).to eq(
          "attachment; filename=\"#{position[:position]}_ddahs.csv\"")
      end
    end
    context "when no offers for position" do
      it "returns a status 404 error" do
        get :ddahs, params: {position_id: no_offer_position[:id]}
        message = {message: "Warning: There are currenly no offers for this position. Operation aborted"}
        expect(response.status).to eq(404)
        expect(response.body).to eq(message.to_json)
      end
    end
    context "when position does not exist" do
      it "returns a status 404 error" do
        get :ddahs, params: {position_id: 'poops'}
        message = {message: "Warning: There is no such position"}
        expect(response.status).to eq(404)
        expect(response.body).to eq(message.to_json)
      end
    end
