...
describe EnrollmentUpdater do
  test_filename="temp"
  subject {
    data = File.read("#{Rails.root}/spec/support/enrollment_data/#{test_filename}.txt")
    updater = EnrollmentUpdater.new(data)
  ...
  success = {updated: true, message: "Enrollment update success."}

  context "well-formatted data" do
    before :each do
      expect(position[:current_enrollment]).to eq(50)
      expect(position[:cap_enrollment]).to eq(nil)
    ...
    it "return update true with message" do
      ...
      position.reload
      expect(position[:current_enrollment]).to eq(196)
      expect(position[:cap_enrollment]).to eq(196)
