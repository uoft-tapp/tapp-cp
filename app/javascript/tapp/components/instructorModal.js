        ...
        if (modalData) {
            ...
            return (
                <Modal
                    id="instructor-modal"
                    show={true}
                    onHide={() => this.props.hideInstructorModal()}
                >
                    <Form horizontal>
                        <Modal.Header>
                            <Modal.Title>
                                ...
                                >
                                    <i
                                        ...
                                        style={{ fontSize: "18px" }}
                                        onClick={() =>
                                            this.props.hideInstructorModal()
                                        }
                                    />
                                </span>
                            </Modal.Title>
                        ...
                        <Modal.Body style={{ paddingTop: "0" }}>
                            <Tab.Container
                                ...
                                onSelect={key =>
                                    this.props.setSelectedTabFromModal(key)
                                }
                            >
                                <Row className="clearfix">
                                    <Col sm={12}>
                                        ...
                                        <Alert
                                            bsStyle="danger"
                                            style={{
                                                opacity: this.props.getInstructorModalAlert()
                                                    ? "1"
                                                    : "0"
                                            }}
                                        >
                                            <b>Error: </b>
                                            {this.props.getInstructorModalAlert()}
                                        </Alert>
                                    </Col>
                                    <Col sm={12}>
                                        <Tab.Content
...
const CreateInstructor = props => (
    <Tab.Pane eventKey="create">
        <FormFormat
            cid="instructor-utorid"
            type="text"
            label="Utorid"
            placeholder="Utorid"
            value={props.getInstructorDataFromModal("utorid")}
            update={event =>
                props.setInstructorDataFromModal("utorid", event.target.value)
            }
            {...props}
        />
...
const EditInstructor = props => (
    <Tab.Pane eventKey="edit">
        <FormFormat
            cid="instructor-utorid-dropdown"
            type="dropdown"
            label="Instructor"
            placeholder={
                props.id
                    ? props.instructors[props.id]["utorid"]
                    : "Choose a Utorid"
            }
...
const InstructorForm = props => (
    <div>
        <FormFormat
            ...
            update={event =>
                props.setInstructorDataFromModal("name", event.target.value)
            }
            {...props}
        />
        <FormFormat
            cid="instructor-email"
            type="email"
            label="Email"
            placeholder="Email"
            value={props.getInstructorDataFromModal("email")}
            update={event =>
                props.setInstructorDataFromModal("email", event.target.value)
            }
            {...props}
        />
    </div>
            ...
            {props.type == "dropdown" ? (
                <div id="instructor-modal-utorid-row">
                    ...
                    <i
                        ...
                        onClick={() => props.deleteInstructor()}
                    />
                </div>
            ) : (
                <FormControl
